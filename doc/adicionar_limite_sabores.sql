-- Adicionar campo max_sabores na tabela produtos
-- Este campo define quantos sabores podem ser selecionados para cada produto

-- Adicionar coluna max_sabores (se não existir)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'produtos' AND column_name = 'max_sabores'
    ) THEN
        ALTER TABLE produtos ADD COLUMN max_sabores INTEGER DEFAULT 1;
    END IF;
END $$;

-- Atualizar produtos existentes com limites baseados no tipo
UPDATE produtos 
